...
class FindSlubCache(gdb.Command):
    ...
    def __init__(self):
        super(FindSlubCache, self).__init__("slub-dumper", gdb.COMMAND_USER)
    ...
    def invoke(self, arg, from_tty):
        ...
        while True:
            ...
            if debug:
                print("[debug] Parse list:", hex(list_offset))
                print("[debug] Parse name:", hex(name_offset))
            ...
            now_kmem_cache_name = gdb.execute(f"x/1s {hex(now_kmem_cache_name_ptr)}", to_string=True).split()[-1]
